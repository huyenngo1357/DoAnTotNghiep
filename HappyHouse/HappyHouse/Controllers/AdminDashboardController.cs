    ...
    public class AdminDashboardController : AdminBaseController
    {
        public ActionResult Index()
        {
            ...
            ViewBag.TongPhongDaThue = db.PhongTroes
                .Count(x => x.TrangThaiPhong == "DaThue"
                          && x.TrangThai == true
                          && x.ToaNha.TrangThaiDuyet == "DaDuyet");
            ...
            ViewBag.ThangRevenue = string.Join(",",
                Enumerable.Range(1, 12)
                    .Select(m => theoThang.ContainsKey(m)
                                 ? theoThang[m] : 0));
            ...
            ViewBag.QuyLabels = string.Join(",",
                Enumerable.Range(1, 4)
                    .Select(q => "\"Q" + q + "/" + nam + "\""));
            ViewBag.QuyRevenue = string.Join(",",
                Enumerable.Range(1, 4).Select(q =>
                {
                    var months = Enumerable.Range((q - 1) * 3 + 1, 3).ToList();
                    return months.Sum(m => theoThang.ContainsKey(m)
                                          ? theoThang[m] : 0);
                }));
